...
# turn back into 5-character string for later file finding

station_id = arctic.STAID
...
# for some of these weather stations, there is no temp data -- e.g. Barkestad (staid 178) has only snow depth recording
# so need to filter out the ones there is no temp data for
...
for staid in station_id_chars:
    ...
    if filename in files:
        station_id_final.append(staid)

len(station_id_final)
...
for staid in station_id_final[1:3]:
    start = time.time()
    counter_percent = counter_percent + round((counter/len(station_id_chars) * 100), 0)
    print(f'working on station {staid}, {counter_percent}% done')
    counter += 1
    filename = 'TG_STAID' + staid + '.txt'
    weather = pd.read_csv('all_weather_data/' + filename, sep = ',', skiprows = 20)
    # weather = pd.read_csv(f'all_weather_data/TG_STAID{staid}.txt', sep = ',', skiprows = 20)
    all_arctic_weather.append(weather)
    # print(f'len of weather is ', len(weather))
    single_station_line = arctic.loc[arctic['STAID'] == pd.to_numeric(staid)]
