...
query = 'merkel' + '-filter:retweets'

tweets_processed_for_db = []

# start = pd.to_datetime("2020-08-20", format = '%Y-%m-%d')
end = pd.to_datetime("2020-08-22", format = '%Y-%m-%d')
...
def get_tweets(end):
    ...
    '''
    api = tweepy.API(auth)
    tweets = tweepy.Cursor(api.search, q=query, lang = 'de', until=str(end), wait_on_rate_limit=True, count = 100).items(100)
    # logging.critical(f'the type of tweets is: {type(tweets)}')
    return tweets
...
if __name__ == '__main__':
    ...
    auth = authenticate()
    tmp = get_tweets(end)
    # print(f'logging length of tweets in tmp: {cardinality.count(tmp)}')

    for tweet in tmp:
        print(f'text.tweet is: {tweet.text}')

        # logging.critical(f'logging length of tweets in tmp: {cardinality.count(tmp)}')
        # logging.critical(tweet.text)
        new = tweet_to_json(tweet)
        print(f'new[text] is: {new["text"]}')
        print('\n')
