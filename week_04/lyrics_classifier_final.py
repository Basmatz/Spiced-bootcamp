...
# Make text files for all the songs of the artists that the user has entered:
...
for artist in search_results_artists.keys():     
     artists_corpus[artist] = make_lemmatized_corpus(artist)   

kino = artists_corpus["Кино"]

kino_cv = cv.fit_transform(kino)
kino_cv_tf = tf.fit_transform(kino_cv)

df_cv_kino = pd.DataFrame(kino_cv.todense(), columns=cv.get_feature_names())
