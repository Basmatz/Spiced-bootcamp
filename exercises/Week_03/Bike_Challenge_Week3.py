...
# 
# Which factors contribute most to the number of bicycles being checkout out over the course of a given day? How much of an impact does weather have on demand? How does the behavior of casual users differ from subscribers? The value in the count column is the one you need to predict.
...
sn.boxplot(data=train,x='weekend',y='casual')
plt.show()
...
"RMSLE score for casual train %f" %(RMSLE(ytrain_cs,clf_1_cs.predict(xtrain_cs)))


# In[36]:


"RMSLE score for casual test %f" %(RMSLE(ytest_cs,clf_1_cs.predict(xtest_cs)))
...
"RMSLE score for registered train %f" %(RMSLE(ytrain_cr,clf_1_cr.predict(xtrain_cr)))


# In[38]:


"RMSLE score for registered test %f" %(RMSLE(ytest_cr,clf_1_cr.predict(xtest_cr)))
...
("Best params", grid_2_cs.best_params_)
"RMSLE score for casual train %f" %(RMSLE(ytrain_cs,grid_2_cs.best_estimator_.predict(xtrain_cs)))


# In[42]:


"RMSLE score for casual test %f" %(RMSLE(ytest_cs,grid_2_cs.best_estimator_.predict(xtest_cs)))
...
("Best params", grid_3_cs.best_params_)
"RMSLE score for casual train %f" %(RMSLE(ytrain_cs,grid_3_cs.best_estimator_.predict(xtrain_cs)))


# In[47]:


"RMSLE score for casual test %f" %(RMSLE(ytest_cs,grid_3_cs.best_estimator_.predict(xtest_cs)))
...
("Best params", grid_3_cr.best_params_)
"RMSLE score for registered train %f" %(RMSLE(ytrain_cr,grid_3_cr.best_estimator_.predict(xtrain_cr)))


# In[50]:


"RMSLE score for registered test %f" %(RMSLE(ytest_cr,grid_3_cr.best_estimator_.predict(xtest_cr)))
...
("Best params",grid_4_cs.best_params_)
"RMSLE score for casual train %f" %(RMSLE(ytrain_cs,grid_4_cs.best_estimator_.predict(xtrain_cs)))
"RMSLE score for casual test %f" %(RMSLE(ytest_cs,grid_4_cs.best_estimator_.predict(xtest_cs)))
...
("Best params",grid_4_cr.best_params_)
"RMSLE score for registered train %f" %(RMSLE(ytrain_cr,grid_4_cr.best_estimator_.predict(xtrain_cr)))
"RMSLE score for registered test %f" %(RMSLE(ytest_cr,grid_4_cr.best_estimator_.predict(xtest_cr)))
